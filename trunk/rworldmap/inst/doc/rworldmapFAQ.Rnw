...
Science (Cefas), Lowestoft, NR33 OHT, UK. {\tt andy.south at cefas.co.uk}}}
...
\section{What is rworldmap?}
rworldmap is a package for visualising global scale data, concentrating on data 
referenced by country codes or gridded at half degree resolution.

\section{Where can I get rworldmap from ?}
http://code.google.com/p/rworld/downloads/list

\section{How do I install R packages ?}

...

\section{Once installed how do I load the package into R ?}
...
\section{How do I map my own country level data ?}
...
%\pagebreak 
\section{How do I map my own half degree gridded data ?}
...
\section{How do I aggregate half degree gridded data to a country level ?}
...
<<plotSetup>>  
mapParams <- mapHalfDegreeGridToCountries(gridExampleData, addLegend=FALSE)
do.call( addMapLegend, c(mapParams, legendWidth=0.5, legendMar = 2))
@
...
\section{How do I aggregate country level data to global regions ?}
...
sternEnvHealth <- country2Region(inFile=dFexampleCountryData,
...
<<plotSetup>>  
mapByRegion(dFexampleCountryData,
...
\section{How do I alter the appearance of my maps ?}
...
\section{How do I create my own colour palette ?}
...
\section{How do I zoom in on defined regions ?}
...
\section{How do I create map bubble plots ?}
...
\pagebreak 
\section{How can I combine rworldmap with other packages classInt and RColorBrewer ?}
...
sPDF <- joinCountryData2Map(dFexampleCountryData,joinCode = "ISO3",nameJoinColumn = "ISO3V10",mapResolution='coarse')
...
\section{How can I create multi-panel plots ?}

par( mfcol=c(5,2) ) in the code chunk below sets up 10 panels in 5 rows and 2 columns, then mapCountryData() is called from within a loop to add one map to each panel. 
